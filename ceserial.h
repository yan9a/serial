...
inline ceSerial::ceSerial(std::string Device, long BaudRate, long DataSize, char ParityType, float NStopBits)
    : stdbaud(true) {
#ifdef CE_WINDOWS
...
inline ceSerial::~ceSerial() { Close(); }

inline void ceSerial::SetPortName(std::string Device) { port = Device; }

inline std::string ceSerial::GetPort() { return port; }
...
inline long ceSerial::GetDataSize() { return dsize; }
...
inline char ceSerial::GetParity() { return parity; }
...
inline float ceSerial::GetStopBits() { return stopbits; }
...
inline long ceSerial::GetBaudRate() { return baud; }
...
#endif
    hComm = CreateFile(wtext.c_str(), GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING,
...
inline bool ceSerial::WriteChar(const char ch) { return Write(&ch, 1); }
