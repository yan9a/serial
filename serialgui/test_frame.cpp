...
MyFrame::MyFrame(const wxString &title)
    : wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, wxSize(390, 280), wxDEFAULT_FRAME_STYLE ^ wxRESIZE_BORDER),
...
void MyFrame::OnAbout(wxCommandEvent &WXUNUSED(event)) {
    wxMessageBox(wxString::Format(wxT("Serial Communication! \n ") wxT("Author: Yan Naing Aye \n ")
                 ...
                 wxT("About Serial Comm"), wxOK | wxICON_INFORMATION, this);
}

void MyFrame::OnOpen(wxCommandEvent &WXUNUSED(event)) {
    if (com.Open())
        txtRx->AppendText(wxString::Format(wxT("Error opening port %s.\n"), com.GetPort()));
    else
...
void MyFrame::SetDataSize(wxCommandEvent &WXUNUSED(event)) {
    if (com.IsOpened()) {
        ...
        long n = wxGetNumberFromUser(wxT("Enter the data size"), wxT("Data Size"), wxT("Set Data Size"),
                                     com.GetDataSize(), 5, 8);
        if (n >= 0) {
            com.SetDataSize(n);
        }
...
void MyFrame::SetStopBits(wxCommandEvent &WXUNUSED(event)) {
    if (com.IsOpened()) {
        ...
        long n = wxGetNumberFromUser(wxT("Enter the number of stop bits"), wxT("Data Size"), wxT("Set stop bits"),
                                     long(com.GetStopBits()), 1, 2);
        if (n > 0) {
...
void MyFrame::ProcessChar(char ch) { txtRx->AppendText(wxString::Format(wxT("%c"), ch)); }

void MyFrame::ClearText(wxCommandEvent &WXUNUSED(event)) { txtRx->Clear(); }
